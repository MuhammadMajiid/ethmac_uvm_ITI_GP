...
class eth_cov_tx extends uvm_component;
    ...
    cp_txpnt: coverpoint m_wdata iff(m_addr>=WB_BD_MEM_BASE_ADDR && m_addr<=WB_BD_MEM_OFFSET_ADDR && m_addr%2==1) {
        ...
        //  not word aligned
        wildcard bins txpnt_not_div4_mod1 = {32'h????_???1,32'h????_???5,32'h????_???9,32'h????_???d};
        wildcard bins txpnt_not_div4_mod2 = {32'h????_???2,32'h????_???6,32'h????_???a,32'h????_???e};
        wildcard bins txpnt_not_div4_mod3 = {32'h????_???3,32'h????_???7,32'h????_???b,32'h????_???f};
    }
    // =============================================================================
    // Cross cover points
    // =============================================================================
